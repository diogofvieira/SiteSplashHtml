(function(){
  ...
  $document.ready(function () {
    ...
    // =================================================================================
    // Video background stop on scroll
    // =================================================================================
    var video_bg = $(".video-bg");
    if(video_bg.length){
      var video_bg_object = $('#home').data('vide').getVideoObject();
      $window.on("scroll", function() {
        if($window.scrollTop() >= $("#home").height()){
          video_bg_object.pause();
        } else {
          video_bg_object.play();
        }
      });
    }
    ...
    if (slider_main.length) {
      ...
      slider_main.on('click', '[data-scroll-to]', function (e) {
        var it = $(this);
        e.preventDefault(), $("body, html").stop().animate({
          scrollTop: $(it.attr("data-scroll-to")).offset().top
        }, 800);
